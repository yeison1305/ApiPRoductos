...
class ProductModel {
    ...
    static async create(product) {
        ...
        try {
            ...
            const { rows: productRows } = await client.query('INSERT INTO products (title, description, category, type, animal_category, brand_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [product.title, product.description, product.category, product.type, product.animal_category, product.brand_id]);
    ...
    static async delete(id) {
        try {
            const { rowCount } = await connection_1.default.query('DELETE FROM products WHERE id = $1', [id]);
            return (rowCount ?? 0) > 0;
        }
        catch (error) {
